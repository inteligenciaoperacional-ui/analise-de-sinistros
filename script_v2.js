...
if (!firebase.apps.length) {
    firebase.initializeApp({
        apiKey: "AIzaSyBWeij4wsbtpIlo8wjYAVOBF43GlfTbfPM",
        authDomain: "sinistros-jca.firebaseapp.com",
        projectId: "sinistros-jca",
        storageBucket: "sinistros-jca.firebasestorage.app",
        messagingSenderId: "599336565035",
        appId: "1:599336565035:web:ede92814def52be9f26d6e"
    });
}
...
function restaurarFotoNaArea(storageKey, idArea, idPreview) {
    var data = fotosMemoria[storageKey];
    if (!data) return;
    ...
    var ph = area.querySelector(".placeholder-interno");
    if (ph) ph.style.display = "none";
    adicionarBotaoDeletar(idArea, idPreview, storageKey);
}
function restaurarFotos(lista) {
    lista.forEach(function(c) { restaurarFotoNaArea(c[2], c[0], c[1]); });
}
...
window.toggleVisibilidadeEtapa = function(etapa) {
    const sw = document.getElementById('checkPossuiEtapa' + etapa);
    const conteudo = document.getElementById('conteudoEtapa' + etapa);
    const msgVazia = document.getElementById('msgVaziaEtapa' + etapa);
    if (!sw) return;
    if (conteudo) conteudo.style.display = sw.checked ? '' : 'none';
    if (msgVazia) msgVazia.style.display  = sw.checked ? 'none' : 'block';
};
...
async function carregarDadosTelemetria() {
    try {
        ...
        const idxTime = 53, idxAce = 1, idxFreio = 44, idxRPM = 45, idxVel = 51;
    ...
    charts[id] = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                x: { 
                    grid: { display: false },
                    ticks: { font: { size: 9 }, color: '#444', maxRotation: 0, autoSkip: false } 
                }
            }
        }
    });
...
window.salvarEIrParaGPS = () => { 
    ...
    restaurarFotos([['areaCapturaFoto','imgPreview','rel_foto_gps']]);
...
window.irParaGPSConclusao = function() {
    ...
    restaurarFotos([['areaCapturaFotoConclusao','prevGPSConclusao','rel_foto_previa']]);
...
window.irParaEtapa5 = function() {
    ...
    restaurarFotos([['areaFoto50_1','prev50_1','rel_foto_50_1'],['areaFoto50_2','prev50_2','rel_foto_50_2'],['areaFoto50_3','prev50_3','rel_foto_50_3'],['areaFoto50_4','prev50_4','rel_foto_50_4'],['areaFoto50_5','prev50_5','rel_foto_50_5'],['areaFoto50_6','prev50_6','rel_foto_50_6']]);
...
window.irParaEtapa52 = () => { 
    ...
    restaurarFotos([['areaFoto52_1','prev52_1','rel_foto_52_1'],['areaFoto52_2','prev52_2','rel_foto_52_2']]);

window.irParaEtapa6Da52 = () => { 
    const temFoto1 = !!fotosMemoria['rel_foto_52_1'];
    const temFoto2 = !!fotosMemoria['rel_foto_52_2'];
    const deveExibir = (temFoto1 || temFoto2) ? 'Sim' : 'Não';
    if (deveExibir === 'Sim' && (!temFoto1 || !temFoto2)) {
        return alert("⚠️ Etapa 5.2: Ambas as imagens das câmeras devem ser anexadas.");
    }
    ...
    restaurarFotos([['areaFotoHistorico','prevHistorico','rel_foto_historico']]);
...
window.irParaEtapa7 = () => { 
    ...
    restaurarFotos([['areaFotoEscala','prevEscala','rel_foto_escala']]);
...
window.irParaEtapa8 = () => { 
    ...
    restaurarFotos([['areaTreinamento1','prevTreinamento1','rel_foto_treinamento_1'],['areaTreinamento2','prevTreinamento2','rel_foto_treinamento_2']]);
...
window.irParaEtapa9 = () => { 
    ...
    restaurarFotos([['areaSinistro25','prevSinistro25','rel_foto_sinistro_2025'],['areaSinistro26','prevSinistro26','rel_foto_sinistro_2026']]);
...
window.irParaEtapa10 = () => { 
    ...
    restaurarFotos([['areaRec25','prevRec25','rel_foto_rec_2025']]);
...
window.irParaEtapa11 = () => { 
    ...
    restaurarFotos([['areaExc25','prevExc25','rel_foto_exc_2025']]);
...
window.irParaEtapa12 = () => { 
    ...
    restaurarFotos([['areaMultas','prevMultas','rel_foto_multas']]);
...
window.finalizarRelatorio = async function() {
    ...
    if (selectOrientacao) localStorage.setItem('rel_layout_fotos', selectOrientacao.value);
    ...
    try {
        const textKeys = [
            'empresa','prefixo','motorista','data_ocorrido','hora_ocorrido',
            'local_ocorrencia','tipo_evento','origem','destino','data_viagem',
            'horario_viagem','fadiga','ativo','analista','resumo_base',
            'gps_resumo','previa_resumo','txt_fadiga','modelo','ano',
            'tecnologia_adas','velocidade_via','complemento_ocorrencia',
            'exibir_cameras','falha_camera_1','falha_camera_2','layout_fotos',
            'exibir_etapa6','exibir_etapa7','exibir_etapa8','exibir_etapa9',
        ...
        const docRef = await db.collection('sinistros').add(dados);
        const docId = docRef.id;

        await Promise.all(Object.entries(fotosMemoria).map(function(entry) {
            if (!entry[1]) return Promise.resolve();
            return db.collection('sinistros').doc(docId).collection('fotos').doc(entry[0]).set({ data: entry[1] });
        }));

        window.open('relatorio_v2.html?id=' + docId, '_blank');
    } catch (err) {
        console.error('Erro ao salvar:', err);
        alert('⚠️ Erro ao salvar no banco de dados.\n' + err.message);
    } finally {
        if (btn) { btn.disabled = false; btn.innerText = 'GERAR RELATÓRIO PDF'; }
    }
};
